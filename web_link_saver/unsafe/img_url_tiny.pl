...
use MIME::Base64;
use URI::URL;
use HTTP::Tiny; # Note that HexChat-2.12.4 bundled perl do not support https via this module, they just miss ssl support at all
                # activeperl 5.20.* crashes, so do not replace bundled perl with activeperl
...
HexChat::register($script_name, '1.0', 'Automatically downloads image URLs via HTTP::Tiny', \&freehooks);
